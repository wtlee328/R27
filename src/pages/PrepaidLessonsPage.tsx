...
import type { Contract, LessonRecord, Customer } from '../types'
import { cn } from '../lib/utils'

type DetailTab = 'contracts' | 'lessons' | 'monthly-summary'

export default function PrepaidLessonsPage() {
  const { contracts, customers, loading: customersLoading } = useCustomers()
  const { records, loading: recordsLoading } = useLessonRecords()
  ...
  // Years options for filter dropdown
  ...
  const monthOptions = useMemo(() => {
    const opts: { value: number | 'all'; label: string }[] = [
      { value: 'all', label: '全年度 (1-12月)' },
    ]
    for (let m = 1; m <= 12; m++) {
      opts.push({ value: m, label: `${String(m).padStart(2, '0')} 月` })
    }
    return opts
  }, [])
  ...
  const summaryMetrics = useMemo(() => {
    // New Prepaid Contracts Collected in Period
    ...
    // Realized Revenue from Lessons in Period
    ...
    periodLessonRecords.forEach((r) => {
      ...
      if (c && c.totalSessions > 0) {
        ...
        // Fallback estimated price per session
        realizedRevenueTotal += sessions * 1500
      }
    })

    // End-of-Month / Current Unearned Deferred Tuition Liability Balance
  ...
  const monthlyMatrix = useMemo(() => {
    const matrix = Array.from({ length: 12 }, (_, idx) => {
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-stone-900 tracking-tight flex items-center gap-2.5">
            <RiBankCardLine className="w-6 h-6 text-orange-500" />
            預收與銷課管理
          </h1>
          <p className="text-sm text-stone-500 mt-1">
            即時稽核預收學費合約（一次付清/分期付款）與每月銷課金額認列對照
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          iconBg="bg-orange-50"
          subtitle={`一次付退 $${summaryMetrics.lumpSumTotal.toLocaleString()} / 分期 $${summaryMetrics.installmentPaidTotal.toLocaleString()}`}
        />
